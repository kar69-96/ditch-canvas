...
const LATEST_EXTRACTION_FILE = path.join(
  __dirname,
  "..",
  "..",
  "storage",
  "latest-extraction-folder.json",
);
...
const MAX_CONCURRENT_CHECKS =
  Number(process.env.UPDATE_MAX_CONCURRENT_CHECKS) || 8;
...
const UPDATE_DRY_RUN =
  (process.env.UPDATE_DRY_RUN || "false").toLowerCase() !== "false";
const UPDATE_COURSE_IDS = process.env.UPDATE_COURSE_IDS
  ? process.env.UPDATE_COURSE_IDS.split(",")
      .map((id) => id.trim())
      .filter(Boolean)
  : null;
const UPDATE_MAX_COURSES = Number(process.env.UPDATE_MAX_COURSES) || null;
const UPDATE_TEST_DATASET_DIR = path.normalize(
  path.join(STORAGE_DIR, "update test"),
);
const DATE_TOLERANCE_HOURS =
  Number(process.env.UPDATE_DATE_TOLERANCE_HOURS) || 24;
const TITLE_SIMILARITY_THRESHOLD =
  Number(process.env.UPDATE_TITLE_SIMILARITY_THRESHOLD) || 0.8;
const AUTO_UPLOAD_TO_SUPABASE =
  (process.env.AUTO_UPLOAD_TO_SUPABASE || "true").toLowerCase() !== "false";
const AUTO_SYNC_INTEGRATIONS =
  (process.env.AUTO_SYNC_INTEGRATIONS || "true").toLowerCase() !== "false";
const UPDATE_USER_EMAIL =
  process.env.UPDATE_USER_EMAIL || process.env.EXTRACTION_EMAIL || null;

// Supabase client for baseline data - optional, don't crash if not available
let getSupabaseClient = null;
try {
  getSupabaseClient =
    require("../../src/services/integrations/supabase-client").getSupabaseClient;
} catch (error) {
  // Supabase not available - will use local files only
}
...
/**
 * Save update log to Supabase database
 * @param {string} userEmail - User's email address
 * @param {string} runId - Update run identifier
 * @param {string} logType - Type of log: 'summary', 'diff', or 'update'
 * @param {Object} data - Log data to save
 * @param {Object} options - Additional options (dryRun, totalCoursesScanned, etc.)
 */
async function saveLogToSupabase(
  ...
  if (!getSupabaseClient || !userEmail) {
    return null;
  }
  ...
  try {
    ...
    if (userError || !userData) {
      console.warn(
        `  ⚠️  Could not find user for log save: ${normalizedEmail}`,
      );
      return null;
    }
    ...
    const { error: insertError } = await supabase.from("update_logs").insert({
      ...
      total_courses_scanned: options.totalCoursesScanned || 0,
      courses_with_updates: options.coursesWithUpdates || 0,
      data: data,
    });

    if (insertError) {
      console.warn(
        `  ⚠️  Failed to save ${logType} log to Supabase: ${insertError.message}`,
      );
      return null;
    }
    ...
    console.warn(`  ⚠️  Error saving log to Supabase: ${error.message}`);
    return null;
  }
}
...
const {
  extractAssignment,
} = require("../../src/crawler/extractors/assignment-extractor.js");
const {
  extractAnnouncement,
} = require("../../src/crawler/extractors/announcement-extractor.js");
const {
  extractFiles,
} = require("../../src/crawler/extractors/file-extractor.js");
const {
  extractModules,
} = require("../../src/crawler/extractors/module-extractor.js");
const {
  extractPage,
} = require("../../src/crawler/extractors/page-extractor.js");
const {
  classifyCanvasUrl,
} = require("../../src/crawler/utils/url-classifier.js");
...
/**
 * PRIVACY GUARDRAIL: Filter out grade-related data to protect student privacy
 * This function removes any grade, score, or GPA data from Canvas API responses
 * to ensure we never store or calculate student grades.
 */
const GRADE_FIELDS_TO_FILTER = new Set([
  ...
  "entered_score",
  "submission_score",
  "total_activity_time", // Activity tracking
]);

function filterGradeData(data) {
  if (!data || typeof data !== "object") return data;
  if (Array.isArray(data)) {
    return data.map((item) => filterGradeData(item));
  }

  const filtered = {};
  for (const [key, value] of Object.entries(data)) {
    // Skip grade-related fields
    if (GRADE_FIELDS_TO_FILTER.has(key.toLowerCase())) {
      continue;
    }
    // Recursively filter nested objects
    if (value && typeof value === "object") {
      filtered[key] = filterGradeData(value);
    } else {
      filtered[key] = value;
    }
  }
...
function ensureNotWritingToUpdateTest(targetPath) {
  ...
  if (normalizedTarget.startsWith(UPDATE_TEST_DATASET_DIR)) {
    throw new Error(
      `Protected dataset folder "${UPDATE_TEST_DATASET_DIR}" cannot be modified by the update checker.`,
    );
  }
}
...
function normalizeDueDateString(value) {
  ...
  const textDatePattern =
    /([A-Z][a-z]{2,3})\s+(\d{1,2})(?:\s+at\s+(\d{1,2}):(\d{2})(am|pm))?/i;
...
function collectFieldChanges(baselineItem, surfaceItem, contentType) {
  ...
  fields.forEach((field) => {
    let baselineValue = normalizeComparableValue(
      baselineItem?.[field] ??
        baselineItem?.[field === "title" ? "name" : field],
    );
    let surfaceValue = normalizeComparableValue(
      surfaceItem?.[field] ?? surfaceItem?.[field === "title" ? "name" : field],
    );
    ...
    if (baselineValue !== surfaceValue) {
      ...
      if (
        baselineValue !== null &&
        baselineValue !== "" &&
        (surfaceValue === null || surfaceValue === "")
      ) {
      ...
      if (contentType === "pages" && field === "title") {
        const baseTitle1 = String(baselineValue || "")
          .replace(/\s*\([^)]*\)\s*$/, "")
          .trim();
        const baseTitle2 = String(surfaceValue || "")
          .replace(/\s*\([^)]*\)\s*$/, "")
          .trim();
...
function buildBaselineCourseData(
  courseId,
  courseName,
  summaryCourseData = {},
  mappingCourseData = null,
) {
  const normalizedCourseId = String(courseId).trim();
  const baseline = {
    courseFolderName:
      summaryCourseData?.courseFolderName ||
      courseName ||
...
function courseHasUpdates(update) {
  ...
  return Object.keys(CONTENT_TYPE_FIELDS).some(
    (type) => update[type]?.hasUpdates,
  );
}
  ...
  surfaceMap.forEach(({ id, data }) => {
    ...
    if (
      ...
      if (moduleItemMatch) {
        ...
        if (courseId) {
          const wasDiscovered = discoveredModuleItems.some(
            (item) =>
              item.itemId === moduleItemId &&
    ...
    if (!baselineEntry && allBaselineData) {
      ...
      const otherTypes = Object.keys(CONTENT_TYPE_FIELDS).filter(
      ...
      if (surfaceTitle) {
        for (const otherType of otherTypes) {
          ...
          const otherEntry = Object.entries(otherBaselineItems).find(
            ([otherId, otherData]) => {
    ...
    if (!baselineEntry) {
      ...
      if (isFailedRequest(id)) {
        if (process.env.DEBUG_UPDATE === "true") {
          console.log(
            `    ⏭️  Skipping ${contentType} ${id} - was a failed request during extraction`,
          );
        }
    ...
    if (contentType === "assignments" && baselineEntry.data.dueDate) {
      const surfaceDueDateStr = String(data.dueDate || "")
        .toLowerCase()
        .trim();
      if (
        surfaceDueDateStr === "closed" ||
        surfaceDueDateStr.includes("closed")
      ) {
    ...
    const baselineDueDate = normalizeDateForComparison(
      baselineEntry.data.dueDate,
    );
    const surfaceDueDate = normalizeDateForComparison(
      normalizedDueDate || data.dueDate,
    );
    ...
    if (baselineHash !== surfaceHash) {
      ...
      const baselineForComparison = {
        ...baselineEntry.data,
        dueDate: baselineDueDate,
        modifiedDate: normalizeDateForComparison(
          baselineEntry.data.modifiedDate,
        ),
        postDate: normalizeDateForComparison(baselineEntry.data.postDate),
      };
      ...
      if (Object.keys(fieldChanges).length > 0) {
        changedItems.push({
          ...
          title:
            data.title ||
            data.name ||
            baselineEntry.data.title ||
            baselineEntry.data.name ||
            "",
...
function diffCourseData(
  baselineCourseData,
  surfaceCourseSummary,
  failedRequests = {},
) {
  // Get content types dynamically from CONTENT_TYPE_FIELDS
...
async function handleDeepExtractionForCourse(
  page,
  courseId,
  extractionFolder,
  diff,
  ...
  for (const contentType of contentTypes) {
    ...
    console.log(
      `    📥 Extracting ${actionableItems.length} ${label}${actionableItems.length > 1 ? "s" : ""}...`,
    );
      ...
      try {
        const result = await Promise.race([
          deepExtractItem(
            ...
            courseId,
            extractionFolder,
            item.id || item.slug || item.pageSlug || null,
          ),
          new Promise((_, reject) =>
            setTimeout(
              () => reject(new Error("Extraction timeout (30s)")),
              30000,
            ),
          ),
        ]);
        ...
        console.log(
          `      ✅ ${action} ${label}: ${item.title || item.name || identifier}`,
        );
      } catch (error) {
        console.error(
          `      ❌ Failed to extract ${label} ${identifier}: ${error.message}`,
        );
      }
    }
  }
...
function getLatestExtractionFolder() {
  ...
  if (overrideFolder) {
    ...
    if (fs.existsSync(overridePath)) {
      ...
      if (fs.existsSync(mappingDir)) {
        console.log(
          `   📂 Using override extraction folder: ${overrideFolder}`,
        );
  ...
  if (!fs.existsSync(STORAGE_DIR)) {
    throw new Error(
      "No storage/datasets directory found. Run a full extraction first.",
    );
  }
  ...
  if (folders.length === 0) {
    throw new Error(
      "No extraction folders found in storage/datasets. Run a full extraction first.",
    );
  }
  ...
  console.log(
    `   📂 Using most recent extraction folder as baseline: ${latestFolder}`,
  );
  return latestFolder;
}
    ...
    for (const moduleFile of moduleFiles) {
      try {
        const moduleData = JSON.parse(
          fs.readFileSync(path.join(modulesDir, moduleFile), "utf8"),
        );
...
function loadRequestQueueRedirects(extractionFolder) {
  ...
  const queueDirs = fs
    .readdirSync(requestQueuesDir, { withFileTypes: true })
    .filter(
      (dirent) => dirent.isDirectory() && dirent.name.startsWith("mapping-"),
    )
    .map((dirent) => dirent.name);
    ...
    for (const queueFile of queueFiles) {
      try {
        const queueData = JSON.parse(
          fs.readFileSync(path.join(queuePath, queueFile), "utf8"),
        );
        const jsonData =
          typeof queueData.json === "string"
            ? JSON.parse(queueData.json)
            : queueData.json;

        if (
          jsonData.url &&
          jsonData.loadedUrl &&
          jsonData.url !== jsonData.loadedUrl
        ) {
          // Extract course ID from URL
...
function loadLatestMappingData(extractionFolder) {
  ...
  const { redirectMap, assignmentRedirects } =
    loadRequestQueueRedirects(extractionFolder);
  ...
  for (const file of files) {
    try {
      ...
      if (
        !data.classification ||
        !relevantTypes.includes(data.classification)
      ) {
        continue;
      }
      ...
      const typeKey =
        type === "assignment"
          ? "assignments"
          : type === "announcement" || type === "announcements"
            ? "announcements"
      ...
      if (typeKey && mappingData[courseId][typeKey]) {
          ...
          if (typeKey === "assignments") {
            ...
            if (moduleMatch) {
              ...
              if (itemMatch) {
                ...
                if (
                  moduleItemMap[normalizedCourseId] &&
                  moduleItemMap[normalizedCourseId][moduleItemId]
                ) {
        ...
        const normalizedCourseId = String(courseId).trim();
        const loadedUrl =
          redirectMap[normalizedCourseId] &&
      ...
      console.warn(
        `Warning: Could not parse mapping file ${file}: ${error.message}`,
      );
    }
  }
  ...
  for (const dir of courseDirs) {
      ...
      if (files.length > 0) {
        try {
          const firstFile = JSON.parse(
            fs.readFileSync(path.join(assignmentsDir, files[0]), "utf8"),
          );
          if (
            String(firstFile.courseId || "").trim() === String(courseId).trim()
          ) {
    ...
    if (fs.existsSync(announcementsDir)) {
      ...
      if (files.length > 0) {
        try {
          const firstFile = JSON.parse(
            fs.readFileSync(path.join(announcementsDir, files[0]), "utf8"),
          );
          if (
            String(firstFile.courseId || "").trim() === String(courseId).trim()
          ) {
...
async function getCourseNameFromCanvas(page, courseId) {
  try {
    ...
    console.warn(
      `  ⚠️  Could not get course name from Canvas for ${courseId}: ${error.message}`,
    );
    return null;
  }
}
...
async function quickCheckAnnouncements(page, courseId) {
  try {
        ...
        if (
          ...
          if (match) {
            ...
            if (!announcementIds.has(id)) {
              ...
              const listItem = link.closest(
                ".discussion, .announcement, .discussion-list-item, .discussion-topic, li, tr, .ig-list-item",
              );
              let postDate = null;
              let author = null;
              ...
              if (listItem) {
                ...
                const lastReplyEl = listItem.querySelector(
                  '.last-reply, .last-reply-date, [data-testid="last-reply"]',
                );
                if (lastReplyEl) {
                  lastReplyDate =
                    lastReplyEl.getAttribute("datetime") ||
                ...
                for (const selector of modifiedSelectors) {
                  ...
                  if (modifiedEl) {
                    modifiedDate =
                      modifiedEl.getAttribute("datetime") ||
    ...
    console.error(
      `  ⚠️  Error checking announcements for course ${courseId}: ${error.message}`,
    );
...
async function quickCheckAssignments(page, courseId) {
  try {
        ...
        if (
          ...
          if (match) {
            ...
            if (!assignmentIds.has(id)) {
              ...
              const listItem = link.closest(
                ".assignment, .assignment-list-item, li, tr, .ig-list-item",
              );
              let dueDate = null;
              let points = null;
              let status = null;
              ...
              if (listItem) {
                ...
                const timeElements =
                  listItem.querySelectorAll("time[datetime]");
                ...
                for (const timeEl of timeElements) {
                  ...
                  if (datetime) {
                    ...
                    const grandparentText = (
                      parentEl?.parentElement?.textContent || ""
                    ).toLowerCase();
                    const title =
                      timeEl.getAttribute("title")?.toLowerCase() || "";
                    const ariaLabel =
                      timeEl.getAttribute("aria-label")?.toLowerCase() || "";
                    const contextText = (
                      parentText +
                      " " +
                      grandparentText +
                    ...
                    const isAvailability =
                      contextText.includes("available until") ||
                      contextText.includes("available from") ||
                      (contextText.includes("available") &&
                        contextText.includes("until"));

                    // Check for explicit due date indicators
                    const isDueDate =
                      contextText.includes("due") &&
                      !contextText.includes("available");

                    // Check for Canvas-specific class names that indicate due dates
                    const hasDueClass = timeEl.closest(
                      '.due-date, .assignment-due-date, .due-date-display, [data-testid="due-date"]',
                    );
                ...
                if (!dueDate && listItem.tagName === "TR") {
                  ...
                  if (!dueDate) {
                    for (const cell of cells) {
                      ...
                      if (
                        cellText.includes("available until") ||
                        cellText.includes("available from")
                      ) {
                        continue;
                      }
                      ...
                      if (timeEl) {
                        ...
                        if (datetime) {
                          ...
                          if (
                            !timeContext.includes("available until") &&
                            !timeContext.includes("available from")
                          ) {
                            dueDate = datetime;
                            break;
                          }
                ...
                if (!dueDate) {
                  const dueContainer = listItem.querySelector(
                    ".assignment-date-due, .ig-details__item.assignment-date-due, .assignment-date",
                  );
                  if (dueContainer) {
                    const dueSpan = dueContainer.querySelector(
                      "span[data-html-tooltip-title], span[title], time[datetime]",
                    );
                    if (dueSpan) {
                      const tooltipDate =
                        dueSpan.getAttribute("data-html-tooltip-title") ||
                        dueSpan.getAttribute("title");
                      if (
                        tooltipDate &&
                        !tooltipDate.toLowerCase().includes("available")
                      ) {
                        dueDate = tooltipDate;
                      } else {
                        const datetime =
                          dueSpan.getAttribute("datetime") ||
                          dueSpan.getAttribute("data-date") ||
                          dueSpan.textContent.trim();
                        if (
                          datetime &&
                          !datetime.toLowerCase().includes("available")
                        ) {
                          dueDate = datetime;
                        }
                      }
                      ...
                      if (
                        strongLabel &&
                        strongLabel.textContent.toLowerCase().includes("due")
                      ) {
                        ...
                        if (sibling) {
                          ...
                          if (
                            possibleDate &&
                            !possibleDate.toLowerCase().includes("available")
                          ) {
                ...
                if (!dueDate) {
                  ...
                  const dataDateEls = listItem.querySelectorAll(
                    "[data-date], [data-due-date], [data-due-at]",
                  );
                  for (const el of dataDateEls) {
                    ...
                    if (dataDate) {
                      ...
                      if (
                        !context.includes("available until") &&
                        !context.includes("available from")
                      ) {
                        try {
                ...
                if (!dueDate) {
                  ...
                  for (const timeEl of allTimeElements) {
                    ...
                    if (datetime) {
                      ...
                      const isAvailability =
                        fullContext.includes("available until") ||
                        fullContext.includes("available from") ||
                        (fullContext.includes("available") &&
                          fullContext.includes("until"));

                      if (!isAvailability) {
                        // Check if it's explicitly a due date (higher priority)
                        const isExplicitDue =
                          fullContext.includes("due") &&
                          !fullContext.includes("available");
                ...
                // Try to find points
                const pointsEl = listItem.querySelector(
                  '.points, .assignment-points, [data-testid="points"]',
                );
                if (pointsEl) {
                ...
                // Try to find status
                const statusEl = listItem.querySelector(
                  '.assignment-status, .status, [data-testid="status"]',
                );
                if (statusEl) {
                ...
                const modifiedEl = listItem.querySelector(
                  '.modified-date, .updated-date, [data-testid="modified"]',
                );
                if (modifiedEl) {
                  modifiedDate =
                    modifiedEl.getAttribute("datetime") ||
    ...
    console.error(
      `  ⚠️  Error checking assignments for course ${courseId}: ${error.message}`,
    );
    return { count: 0, items: [] };
  }
}
  ...
  try {
    ...
    const data = await page.evaluate(() => {
      ...
      const rows = document.querySelectorAll(
        '.ef-directory .ef-item-row, .ef-folder-list [role="treeitem"]',
      );
      ...
      rows.forEach((row) => {
        ...
        if (link) {
          ...
          if (href) {
            ...
            // Try to find size
            const sizeEl = row.querySelector(
              '.ef-size-col, .file-size, [data-testid="file-size"]',
            );
            if (sizeEl) {
            ...
            // Try to find modified date
            const modifiedEl = row.querySelector(
              '.ef-date-col, .modified-date, .file-modified, [data-testid="modified-date"]',
            );
            if (modifiedEl) {
              modifiedDate =
                modifiedEl.getAttribute("datetime") ||
            ...
            if (
              href.includes("/files/folder/") ||
              row.classList.contains("ef-folder")
            ) {
              folders.push({
                url: fullUrl,
                name: name,
    ...
    console.error(
      `  ⚠️  Error checking files for course ${courseId}: ${error.message}`,
    );
...
async function quickCheckModules(page, courseId) {
  try {
    ...
    await waitForListRender(
      page,
      [".context_module", '[data-testid="module"]', ".module"],
      1500,
    );
    ...
    const data = await page.evaluate(() => {
      ...
      moduleElements.forEach((moduleEl) => {
        ...
        if (moduleId) {
          ...
          const nameEl = moduleEl.querySelector(
            ".ig-header-title, .module-name, h3, h2",
          );
          const name = nameEl ? nameEl.textContent.trim() : null;

          // Count items in module
          const itemCount = moduleEl.querySelectorAll(
            ".ig-list-item, .module-item",
          ).length;

          // Try to find unlock date
          let unlockDate = null;
          const unlockEl = moduleEl.querySelector(
            '.unlock-date, .module-unlock, [data-testid="unlock-date"]',
          );
          if (unlockEl) {
            unlockDate =
              unlockEl.getAttribute("datetime") ||
          ...
          const completionEl = moduleEl.querySelector(
            ".module-completion, .completion-status",
          );
          if (completionEl) {
    ...
    console.error(
      `  ⚠️  Error checking modules for course ${courseId}: ${error.message}`,
    );
    return { count: 0, items: [] };
  }
}
...
async function quickCheckPages(page, courseId) {
  try {
        ...
        if (href && href.includes("/pages/")) {
          ...
          if (match) {
            ...
            if (!pageSlugs.has(slug)) {
              ...
              if (!title) {
                const fallbackEl = link.querySelector(
                  '[data-testid="page-title"], .ellipsible, span, strong',
                );
                if (fallbackEl) {
    ...
    console.error(
      `  ⚠️  Error checking pages for course ${courseId}: ${error.message}`,
    );
    return { count: 0, items: [] };
  }
}
...
function createItemHash(item, contentType) {
  ...
  const normalizeDate = (date) => {
    ...
    const canvasTextMatch = dateStr.match(
      /(?:available until|due|until)?\s*([A-Z][a-z]{2,3})\s+(\d{1,2}),?\s*(\d{4})?/i,
    );
    if (canvasTextMatch) {
      const monthNames = [
        "jan",
    ...
    if (
      lowerStr.includes("closed") ||
      lowerStr.includes("no due date") ||
      lowerStr.includes("tomorrow") ||
      lowerStr.includes("yesterday")
    ) {
  ...
  const hashData = {
    ...
    points: item.points
      ? normalizeString(item.points).replace(/[^\d.]/g, "")
      : null,
  };
...
function compareMappingData(
  ...
  if (
    storedExtracted &&
    typeof storedExtracted === "object" &&
    !Array.isArray(storedExtracted)
  ) {
  ...
  if (process.env.DEBUG_UPDATE === "true" && storedExtractedById.size > 0) {
    ...
    console.log(
      `    🔍 Stored ${contentType} IDs (sample): ${sampleIds.join(", ")}`,
    );
  }
  ...
  if (contentType === "files") {
    ...
    const storedUrls = new Set(
      storedItems.map((item) => item.url).filter(Boolean),
    );
    ...
    currentItems.items.forEach((item) => {
      ...
      if (storedExtractedItem) {
        ...
        const currentItemForHash = {
          ...
          ...(item.postDate !== null &&
            item.postDate !== undefined && { postDate: item.postDate }),
          ...(item.points !== null &&
        ...
        const storedItemForHash = {
          id:
            storedExtractedItem.id ||
            (contentType === "assignments"
              ? storedExtractedItem.assignmentId
              : contentType === "announcements"
                ? storedExtractedItem.announcementId
                : contentType === "modules"
                  ? storedExtractedItem.moduleId
                  : contentType === "pages"
                    ? storedExtractedItem.pageSlug
                    : contentType === "files"
                      ? storedExtractedItem.fileId
                      : normalizedId),
          ...
          ...(currentItemForHash.dueDate !== undefined && {
            dueDate: storedExtractedItem.dueDate,
          }),
          ...(currentItemForHash.modifiedDate !== undefined && {
            modifiedDate:
              storedExtractedItem.modifiedDate || storedExtractedItem.updatedAt,
          }),
          ...(currentItemForHash.postDate !== undefined && {
            postDate:
              storedExtractedItem.postDate ||
              storedExtractedItem.postedAt ||
              storedExtractedItem.createdAt,
          }),
          ...(currentItemForHash.points !== undefined && {
            points:
              storedExtractedItem.points || storedExtractedItem.pointsPossible,
          }),
          ...(currentItemForHash.slug !== undefined && {
            slug: storedExtractedItem.pageSlug || storedExtractedItem.slug,
          }),
        };
      ...
      if (!storedExtractedItem) {
        ...
        if (process.env.DEBUG_UPDATE === "true") {
          console.log(
            `    ➕ Item ${normalizedId} not in extracted data - treating as new`,
          );
        }
      }
    });
 ...
 */
async function deepExtractItem(
  page,
  url,
  contentType,
  courseId,
  ...
  try {
          ...
          if (fs.existsSync(announcementsDir)) {
            ...
            if (files.length > 0) {
              try {
                const firstFile = JSON.parse(
                  fs.readFileSync(
                    path.join(announcementsDir, files[0]),
                    "utf8",
                  ),
                );
                if (firstFile.courseId === courseId) {
    ...
    const datasetBasePath = path.join(
      STORAGE_DIR,
      extractionFolder,
      "datasets",
      datasetPrefix,
    ...
    if (contentType === "assignments") {
      ...
      if (
        extractorResult &&
        extractorResult.files &&
        extractorResult.files.length > 0
      ) {
      ...
      if (!itemId) {
        ...
        if (match) {
          ...
          extractorResult &&
          extractorResult.modules &&
          extractorResult.modules.length > 0
        ) {
    ...
    if (contentType === "modules") {
      ...
      if (fs.existsSync(modulesDir)) {
        ...
        if (files.length > 0) {
          ...
          fs.writeFileSync(
            existingFile,
            JSON.stringify(extractorResult, null, "\t"),
          );
          return {
            ...extractorResult,
            _updated: true,
            _filePath: existingFile,
          };
        }
      }
    ...
    const existingFile = itemId
      ? findExistingItemFile(
          courseFolderName,
          contentType,
          itemId,
          extractionFolder,
    ...
    if (existingFile) {
      ...
      if (filePath && fs.existsSync(filePath)) {
        ...
        ensureDirectoryExists(fileDir);
        ensureNotWritingToUpdateTest(filePath);
        // Apply grade filter before saving to protect student privacy
        const filteredResult = filterGradeData(extractorResult);
        fs.writeFileSync(filePath, JSON.stringify(filteredResult, null, "\t"));
      }

      return { ...filterGradeData(extractorResult), _created: true };
    }
  } catch (error) {
    console.error(
      `  ❌ Error extracting ${contentType} from ${url}: ${error.message}`,
    );
    throw error;
  }
}
...
function loadExtractionSummary(extractionFolder) {
  ...
  if (!fs.existsSync(summaryPath)) {
    ...
    console.warn(
      `  💡 Run: node scripts/generate-extraction-summary.js "${extractionFolder}"`,
    );
    return null;
  }
...
/**
 * Load baseline data from Supabase for a user
 * Returns data in the same format as extraction summary
 * @param {string} userEmail - User's email address
 * @returns {Promise<Object|null>} - Summary-like object or null
 */
async function loadBaselineFromSupabase(userEmail) {
  ...
  if (!userEmail) {
    console.warn("  ⚠️  No user email provided for Supabase baseline");
    return null;
  }
  ...
  try {
    ...
    // First, look up the user ID from email
    const { data: userData, error: userError } = await supabase
      .from("users")
      .select("id")
      .eq("email", normalizedEmail)
    ...
    if (userError || !userData) {
      console.warn(`  ⚠️  User not found in Supabase for ${normalizedEmail}`);
      return null;
    }

    const userId = userData.id;

    // Try RPC function first, fall back to direct table query
    let entities = null;
    let queryError = null;
    ...
    if (!rpcError && rpcData) {
      ...
      // Fallback: direct query on extraction_data table
      console.log(
        "  ℹ️  RPC unavailable, querying extraction_data table directly...",
      );
    ...
    if (!entities || entities.length === 0) {
      console.warn(
        `  ⚠️  No extraction data found in Supabase for ${normalizedEmail}`,
      );
      return null;
    }

    // Group entities by course and type
    const courses = {};

    entities.forEach((entity) => {
      const courseId = String(entity.course_id || "unknown").trim();
      ...
      if (!courses[courseId]) {
        courses[courseId] = {
          courseId,
          courseFolderName: data.courseName || `course-${courseId}`,
          courseName: data.courseName || `Course ${courseId}`,
          assignments: {},
          announcements: {},
          files: {},
          modules: {},
      ...
      // Map entity to the right category
      const typeMap = {
      ...
      const category = typeMap[entityType];
      if (category && entityId) {
        courses[courseId][category][entityId] = {
          id: entityId,
          title: data.title || data.name || "",
          url: data.url || "",
          dueDate: data.dueAt || data.dueDate || null,
          modifiedDate: data.updatedAt || data.modifiedDate || null,
          points: data.pointsPossible || data.points || null,
          postDate: data.postedAt || data.postDate || null,
    ...
    console.log(
      `  ✅ Loaded ${entities.length} entities across ${Object.keys(courses).length} courses from Supabase`,
    );
    return summary;
  } catch (error) {
...
async function uploadToSupabase(extractionFolder, summary) {
  if (!AUTO_UPLOAD_TO_SUPABASE) {
    console.log(
      "💤 Auto-upload to Supabase is disabled (set AUTO_UPLOAD_TO_SUPABASE=true to enable)\n",
    );
  ...
  if (!userEmail) {
    console.warn(
      "⚠️  Cannot auto-upload to Supabase: No user email found in extraction summary\n",
    );
  ...
  try {
    ...
    // 1. Frontend directory (local development - root/../frontend)
    // 2. Frontend directory (AWS - root is in ~/Canvas-Wrapper, frontend is in ~/Canvas-Wrapper/../frontend)
    ...
    if (!uploadScriptPath) {
      const isAWS =
        process.env.AWS_INSTANCE_ID || process.env.HEADLESS === "true";
    ...
    if (
      ...
      if (isAWS) {
        console.warn(
          "⚠️  Running on AWS instance - frontend directory not found",
        );
        ...
        console.warn(
          `   Or download data and run locally: npm run supabase:upload-data ${userEmail} "${extractionDataPath}"\n`,
        );
        return {
          success: false,
          error: "Frontend directory not found on AWS",
          needsLocalUpload: true,
        };
      }

      console.warn(
        `⚠️  Frontend directory not found (needed for npm run supabase:upload-data)`,
      );
...
async function triggerIntegrationSyncs() {
  if (!AUTO_SYNC_INTEGRATIONS) {
    console.log(
      "💤 Auto-sync integrations is disabled (set AUTO_SYNC_INTEGRATIONS=true to enable)\n",
    );
    return { success: false, skipped: true };
  }
  ...
  console.log(
    "🔄 Syncing integrations (Google Sheets, Notion, Google Calendar)...\n",
  );
  ...
  try {
    ...
    if (result.ran > 0) {
      console.log(
        `✅ Successfully synced ${result.success || 0}/${result.ran} integration(s)\n`,
      );
...
function getStoredExtractedDataFromSummary(summary, courseId) {
  ...
  if (!courseData) {
    if (process.env.DEBUG_UPDATE === "true") {
      console.log(
        `    ⚠️  Course ${normalizedCourseId} not found in summary. Available courses: ${Object.keys(summary.courses).join(", ")}`,
      );
    }
    return {
      assignments: {},
        ...
        quizFiles.forEach((file) => {
          try {
            const quizData = JSON.parse(
              fs.readFileSync(path.join(quizzesDir, file), "utf8"),
            );
 ...
 */
async function checkCourseForUpdates(
  browser,
  courseId,
  extractionFolder,
  summary,
  ...
  try {
    ...
    if (!courseName) {
      console.log(
        `  📖 Course name not found in stored data, fetching from Canvas...`,
      );
    ...
    const surfaceCourseSummary = buildSurfaceCourseSummary(
      courseId,
      fallbackCourseName,
      quickData,
    );
    ...
    const baselineCourseData = buildBaselineCourseData(
      courseId,
      fallbackCourseName,
      baseCourseData,
      mappingCourseData,
    ...
    const diff = diffCourseData(baselineCourseData, surfaceCourseSummary, {
      [normalizedCourseId]: courseFailedRequests,
    });
    ...
    const hasActionableChanges = Object.values(diff).some(
      (section) => section.hasUpdates,
    );
    ...
    if (hasActionableChanges) {
      ...
      console.log(
        "  📋 Changes will be applied after all courses are checked.",
      );
    } else {
...
function deleteRemovedItems(
  ...
  for (const item of removedItems) {
    ...
    const filePath = findExistingItemFile(
      courseFolderName,
      contentType,
      itemId,
      extractionFolder,
    ...
    if (filePath && fs.existsSync(filePath)) {
      try {
        ...
        if (process.env.DEBUG_UPDATE === "true") {
          console.log(
            `      🗑️  Deleted ${contentType}: ${item.title || item.name || itemId}`,
          );
        }
      } catch (error) {
        console.error(
          `      ⚠️  Failed to delete ${contentType} ${itemId}: ${error.message}`,
        );
      }
    }
  }
 ...
 */
async function saveUpdateLog(diffReport, extractionFolder, changesApplied) {
    ...
    courses: diffReport.courses.map((course) => ({
      ...
      changes: {
        announcements: {
          ...
          items: {
            new:
              course.announcements.newItems?.map((item) => ({
                id: item.id,
                title: item.title,
                url: item.url,
              })) || [],
            changed:
              course.announcements.changedItems?.map((item) => ({
                id: item.id,
                title: item.title,
                url: item.url,
            ...
            removed:
              course.announcements.removedItems?.map((item) => ({
                id: item.id,
                title: item.title,
                url: item.url,
              })) || [],
        ...
        assignments: {
          ...
          items: {
            new:
              course.assignments.newItems?.map((item) => ({
                id: item.id,
                title: item.title,
                url: item.url,
            ...
            changed:
              course.assignments.changedItems?.map((item) => ({
                id: item.id,
                title: item.title,
                url: item.url,
                fieldChanges: item.fieldChanges,
            ...
            removed:
              course.assignments.removedItems?.map((item) => ({
                id: item.id,
                title: item.title,
                url: item.url,
              })) || [],
        ...
        files: {
          ...
          items: {
            new:
              course.files.newItems?.map((item) => ({
                id: item.id,
                name: item.name || item.title,
                url: item.url,
              })) || [],
            changed:
              course.files.changedItems?.map((item) => ({
                id: item.id,
                name: item.name || item.title,
                url: item.url,
                fieldChanges: item.fieldChanges,
            ...
            removed:
              course.files.removedItems?.map((item) => ({
                id: item.id,
                name: item.name || item.title,
                url: item.url,
              })) || [],
        ...
        modules: {
          ...
          items: {
            new:
              course.modules.newItems?.map((item) => ({
                id: item.id,
                title: item.title,
                url: item.url,
              })) || [],
            changed:
              course.modules.changedItems?.map((item) => ({
                id: item.id,
                title: item.title,
                url: item.url,
                fieldChanges: item.fieldChanges,
            ...
            removed:
              course.modules.removedItems?.map((item) => ({
                id: item.id,
                title: item.title,
                url: item.url,
              })) || [],
        ...
        pages: {
          ...
          items: {
            new:
              course.pages.newItems?.map((item) => ({
                id: item.slug || item.id,
                title: item.title,
                url: item.url,
              })) || [],
            changed:
              course.pages.changedItems?.map((item) => ({
                id: item.slug || item.id,
                title: item.title,
                url: item.url,
                fieldChanges: item.fieldChanges,
            ...
            removed:
              course.pages.removedItems?.map((item) => ({
                id: item.slug || item.id,
                title: item.title,
                url: item.url,
              })) || [],
  ...
  if (UPDATE_USER_EMAIL) {
    const saved = await saveLogToSupabase(
      UPDATE_USER_EMAIL,
      diffReport.runId,
      "update",
      updateLog,
    ...
    if (saved) {
      console.log(`   ✅ Update log also saved to Supabase\n`);
    } else {
      console.log(`\n`);
    }
  } else {
 ...
 */
async function applyChangesToExtractionFolder(
  diffReport,
  browser,
  extractionFolder,
) {
  ...
  try {
    ...
    for (const courseUpdate of diffReport.courses) {
      ...
      for (const contentType of contentTypes) {
        ...
        if (section.removedItems && section.removedItems.length > 0) {
          const deletedCount = deleteRemovedItems(
            courseFolderName,
            contentType,
            section.removedItems,
            extractionFolder,
          );
        ...
        if (itemsToExtract.length > 0) {
          ...
          console.log(
            `    📥 Extracting ${itemsToExtract.length} ${label}${itemsToExtract.length > 1 ? "s" : ""}...`,
          );
          ...
          for (const item of itemsToExtract) {
            ...
            if (!item.url) {
              console.log(
                `      ⚠️  Skipping ${label} ${identifier} — missing URL`,
              );
              continue;
            }
      ...
      if (courseChangesApplied > 0) {
        console.log(
          `    ✅ Applied ${courseChangesApplied} change(s) to course ${courseId}\n`,
        );
      } else {
    ...
    try {
      ...
      const summaryScript = path.join(
        __dirname,
        "generate-extraction-summary.js",
      );
      ...
      await new Promise((resolve, reject) => {
        ...
        child.on("close", (code) => {
          if (code === 0) {
            ...
            console.error(
              `   ⚠️  Extraction summary regeneration exited with code ${code}`,
            );
        ...
        child.on("error", (error) => {
          console.error(
            `   ❌ Failed to regenerate extraction summary: ${error.message}`,
          );
          reject(error);
        });
      });
      ...
      console.error(
        `   ⚠️  Could not regenerate extraction summary: ${error.message}`,
      );
      console.error(
        `   💡 You can manually run: node scripts/generate-extraction-summary.js "${extractionFolder}"`,
      );
    }

    console.log(
      `✅ Applied ${totalChangesApplied} total change(s) to extraction folder\n`,
    );

    // Save update log
    const updateLogPath = await saveUpdateLog(
      diffReport,
      extractionFolder,
      totalChangesApplied,
    );
...
async function main() {
  ...
  try {
    // Require user email - Supabase is the only data source
    if (!UPDATE_USER_EMAIL) {
      console.error(
        "❌ No user email specified. Set UPDATE_USER_EMAIL environment variable.",
      );
      console.error(
        "   Example: UPDATE_USER_EMAIL=user@colorado.edu npm run update:dry-run",
      );
      return;
    }

    // Load baseline from Supabase
    console.log("☁️  Loading baseline from Supabase...");
    const summary = await loadBaselineFromSupabase(UPDATE_USER_EMAIL);

    if (!summary) {
      console.error(
        "❌ No extraction data found in Supabase. Run initial extraction first.",
      );
      return;
    }

    const extractionFolder = `supabase-${UPDATE_USER_EMAIL.split("@")[0]}`;

    const updateTestTimestampBefore = getDirectoryTimestamp(
      UPDATE_TEST_DATASET_DIR,
    );
    ...
    console.log(
      "📚 Using extraction summary as baseline (more accurate than mapping data)...",
    );
    ...
    console.log(
      `   ✅ Found ${courseIds.length} courses from extraction summary\n`,
    );

    if (UPDATE_COURSE_IDS && UPDATE_COURSE_IDS.length > 0) {
      courseIds = courseIds.filter((id) => UPDATE_COURSE_IDS.includes(id));
      console.log(
        `   🎯 Filtered to ${courseIds.length} course(s) via UPDATE_COURSE_IDS\n`,
      );
    }

    if (
      UPDATE_MAX_COURSES &&
      UPDATE_MAX_COURSES > 0 &&
      courseIds.length > UPDATE_MAX_COURSES
    ) {
      courseIds = courseIds.slice(0, UPDATE_MAX_COURSES);
      console.log(
        `   ⏱️ Limiting scan to ${courseIds.length} course(s) (UPDATE_MAX_COURSES)\n`,
      );
    }
    ...
    try {
      const supabase = getSupabaseClient();
      const normalizedEmail = UPDATE_USER_EMAIL.toLowerCase().trim();

      const { data: user, error } = await supabase
        .from("users")
        .select("canvas_cookies, canvas_cookies_updated_at")
        .eq("email", normalizedEmail)
        .single();
      ...
      if (
        !user.canvas_cookies ||
        (Array.isArray(user.canvas_cookies) && user.canvas_cookies.length === 0)
      ) {
        console.error(`❌ No cookies found for user: ${normalizedEmail}`);
        console.error("   User needs to login first.");
        return;
      }

      console.log(
        `   ✅ Found cookies (updated: ${user.canvas_cookies_updated_at || "unknown"})`,
      );

      // Write cookies to temp file for browser session
    ...
    try {
      ...
      for (let i = 0; i < courseIds.length; i += MAX_CONCURRENT_CHECKS) {
          ...
          Promise.race([
            ...
            new Promise((_, reject) =>
              setTimeout(
                () =>
                  reject(
                    new Error(`Course ${courseId} check timeout (2 minutes)`),
                  ),
                120000,
              ),
            ),
            ...
            console.error(
              `  ❌ Error checking course ${courseId}: ${error.message}`,
            );
            return {
              updates: createEmptyCourseUpdate(courseId),
              surfaceCourseSummary: buildSurfaceCourseSummary(
                courseId,
                `course-${courseId}`,
                createEmptyQuickData(),
              ),
            };
          }),
        );
        ...
        batchResults.forEach((result) => {
          ...
          surfaceSummary.courses[result.updates.courseId] =
            result.surfaceCourseSummary;
        });

        // Progress indicator
        const processed = Math.min(i + MAX_CONCURRENT_CHECKS, courseIds.length);
        console.log(
          `   ✅ Checked ${processed}/${courseIds.length} courses...\n`,
        );
      }

      const summaryPath = path.join(
        UPDATE_RESULTS_DIR,
        SURFACE_SUMMARY_FILENAME,
      );
      ...
      if (UPDATE_USER_EMAIL) {
        const summarySaved = await saveLogToSupabase(
          UPDATE_USER_EMAIL,
          UPDATE_RUN_ID,
          "summary",
          surfaceSummary,
          { dryRun: UPDATE_DRY_RUN, totalCoursesScanned: courseIds.length },
        );
        if (summarySaved) {
          console.log(`   ✅ Summary also saved to Supabase\n`);
        } else {
          console.log(`\n`);
        }
      } else {
      ...
      // Save diff report to Supabase
      if (UPDATE_USER_EMAIL) {
        const diffSaved = await saveLogToSupabase(
          UPDATE_USER_EMAIL,
          UPDATE_RUN_ID,
          "diff",
          diffReport,
        ...
        if (diffSaved) {
          console.log(`   ✅ Diff report also saved to Supabase\n`);
        } else {
          console.log(`\n`);
        }
      } else {
      ...
      if (!UPDATE_DRY_RUN && coursesWithUpdates.length > 0) {
        ...
        try {
          const applyResult = await applyChangesToExtractionFolder(
            diffReport,
            browser,
            extractionFolder,
          );
          ...
          if (applyResult.success) {
            console.log(
              `✅ Successfully applied ${applyResult.changesApplied} change(s) to extraction folder\n`,
            );
            if (applyResult.updateLogPath) {
              console.log(
                `📝 Update log saved to: ${applyResult.updateLogPath}\n`,
              );
            }
            ...
            if (updatedSummary) {
              ...
              const hasAssignmentUpdates = diffReport.courses.some(
                (course) =>
            ...
            console.error(
              "⚠️  Some changes may not have been applied. Check the logs above.\n",
            );
          }
        } catch (error) {
        ...
        console.log(
          "💤 Dry-run mode: Changes detected but not applied to extraction folder.\n",
        );
        console.log(
          `   To apply changes, run with: UPDATE_DRY_RUN=false node scripts/update.js\n`,
        );
        // Save update log even in dry-run mode (for reference)
        await saveUpdateLog(diffReport, extractionFolder, 0);
        ...
        console.log(
          "📊 No updates found, but ensuring Supabase is synced with latest data...\n",
        );
      ...
      if (coursesWithUpdates.length === 0) {
        ...
        console.log(
          `⚠️  Found updates in ${coursesWithUpdates.length} course(s):\n`,
        );
        ...
        for (const update of coursesWithUpdates) {
          ...
          if (update.announcements.hasUpdates) {
            ...
            console.log(
              `   📢 Announcements: +${newCount} new, ~${changedCount} changed, -${removedCount} removed`,
            );
            if (update.announcements.newItems.length > 0) {
              ...
              if (update.announcements.newItems.length > 3) {
                console.log(
                  `        ... and ${update.announcements.newItems.length - 3} more`,
                );
              }
            }
            if (update.announcements.changedItems.length > 0) {
              ...
              if (update.announcements.changedItems.length > 3) {
                console.log(
                  `        ... and ${update.announcements.changedItems.length - 3} more`,
                );
              }
            }
          }
          ...
          if (update.assignments.hasUpdates) {
            ...
            console.log(
              `   📝 Assignments: +${newCount} new, ~${changedCount} changed, -${removedCount} removed`,
            );
            if (update.assignments.newItems.length > 0) {
              ...
              if (update.assignments.newItems.length > 3) {
                console.log(
                  `        ... and ${update.assignments.newItems.length - 3} more`,
                );
              }
            }
            if (update.assignments.changedItems.length > 0) {
              ...
              if (update.assignments.changedItems.length > 3) {
                console.log(
                  `        ... and ${update.assignments.changedItems.length - 3} more`,
                );
              }
            }
          }
          ...
          if (update.files.hasUpdates) {
            ...
            console.log(
              `   📁 Files: +${newCount} new, ~${changedCount} changed, -${removedCount} removed`,
            );
            if (update.files.newItems.length > 0) {
              ...
              if (update.files.newItems.length > 3) {
                console.log(
                  `        ... and ${update.files.newItems.length - 3} more`,
                );
              }
            }
            if (update.files.changedItems.length > 0) {
              ...
              if (update.files.changedItems.length > 3) {
                console.log(
                  `        ... and ${update.files.changedItems.length - 3} more`,
                );
              }
            }
          }
          ...
          if (update.modules.hasUpdates) {
            ...
            console.log(
              `   📦 Modules: +${newCount} new, ~${changedCount} changed, -${removedCount} removed`,
            );
            if (update.modules.newItems.length > 0) {
              ...
              if (update.modules.newItems.length > 3) {
                console.log(
                  `        ... and ${update.modules.newItems.length - 3} more`,
                );
              }
            }
            if (update.modules.changedItems.length > 0) {
              ...
              if (update.modules.changedItems.length > 3) {
                console.log(
                  `        ... and ${update.modules.changedItems.length - 3} more`,
                );
              }
            }
          }
          ...
          if (update.pages.hasUpdates) {
            ...
            console.log(
              `   📄 Pages: +${newCount} new, ~${changedCount} changed, -${removedCount} removed`,
            );
            if (update.pages.newItems.length > 0) {
              ...
              if (update.pages.newItems.length > 3) {
                console.log(
                  `        ... and ${update.pages.newItems.length - 3} more`,
                );
              }
            }
            if (update.pages.changedItems.length > 0) {
              ...
              if (update.pages.changedItems.length > 3) {
                console.log(
                  `        ... and ${update.pages.changedItems.length - 3} more`,
                );
              }
            }
          }
      ...
      const updateTestTimestampAfter = getDirectoryTimestamp(
        UPDATE_TEST_DATASET_DIR,
      );
      if (
        updateTestTimestampBefore !== null &&
        updateTestTimestampAfter !== null
      ) {
        if (updateTestTimestampAfter !== updateTestTimestampBefore) {
          console.warn(
            "⚠️  storage/datasets/update test timestamp changed during the run. Verify no writes occurred.",
          );
        } else {
