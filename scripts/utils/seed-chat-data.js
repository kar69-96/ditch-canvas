...
if (!supabaseUrl || !supabaseKey) {
  console.error(
    "Error: VITE_SUPABASE_URL and VITE_SUPABASE_ANON_KEY must be set in .env.local",
  );
  process.exit(1);
}
...
async function getAllCourses() {
  // Try to get courses from extraction_data table directly
  ...
  const dataPath = path.join(
    __dirname,
    "../../frontend/mock-data/extraction-data",
  );
  ...
  console.warn(
    "No courses found. Please ensure courses exist in the database or data files.",
  );
  return [];
}
