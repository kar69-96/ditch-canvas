-- ============================================
-- Seed Sample Chat Posts
-- ============================================
-- This migration inserts 3-5 sample discussion posts
-- with responses for testing the chat forum feature.
-- 
-- INSTRUCTIONS TO CUSTOMIZE:
-- 1. Find the section below marked "CUSTOMIZE THESE VALUES"
-- 2. Replace the course_id values with your actual course IDs
-- 3. Modify post titles, bodies, and tags as needed
-- 4. Adjust the number of posts (currently 5)
-- ============================================
...
BEGIN
  ...
  -- This queries courses stored in the user's data tables
  BEGIN
    -- Get courses from flexible storage
  ...
  IF array_length(sample_course_ids, 1) IS NULL THEN
    RAISE EXCEPTION 'No courses found. Please ensure you have courses in your database. You can manually set course IDs by editing the migration file.';
  END IF;
  ...
  FOR course_idx IN 1..array_length(sample_course_ids, 1) LOOP
    ...
    BEGIN
      ...
      selected_fruit := fruit_names[1 + (course_idx * 2) % array_length(fruit_names, 1)];
      ...
      response_fruit := fruit_names[2 + (course_idx * 2) % array_length(fruit_names, 1)];
      ...
      -- Update response count and net_score for post
      ...
      -- Post 2: Discussion
      selected_fruit := fruit_names[3 + (course_idx * 2) % array_length(fruit_names, 1)];
      INSERT INTO chat_posts (course_id, user_id, anonymous_thread_id, title, body, tag, created_at, updated_at)
      VALUES (
        course_id_val,
        sample_user_id,
      ...
      response_fruit := fruit_names[4 + (course_idx * 2) % array_length(fruit_names, 1)];
      INSERT INTO chat_responses (post_id, user_id, anonymous_thread_id, body, created_at, updated_at)
      VALUES (
        post_record.id,
        sample_user_id,
      ...
      response_fruit := fruit_names[5 + (course_idx * 2) % array_length(fruit_names, 1)];
      INSERT INTO chat_responses (post_id, user_id, anonymous_thread_id, body, created_at, updated_at)
      VALUES (
        post_record.id,
        sample_user_id,
      ...
      selected_fruit := fruit_names[6 + (course_idx * 2) % array_length(fruit_names, 1)];
      INSERT INTO chat_posts (course_id, user_id, anonymous_thread_id, title, body, tag, created_at, updated_at)
      VALUES (
        course_id_val,
        sample_user_id,
      ...
      response_fruit := fruit_names[7 + (course_idx * 2) % array_length(fruit_names, 1)];
      INSERT INTO chat_responses (post_id, user_id, anonymous_thread_id, body, created_at, updated_at)
      VALUES (
        post_record.id,
        sample_user_id,
      ...
      -- Post 4: Discussion
      selected_fruit := fruit_names[8 + (course_idx * 2) % array_length(fruit_names, 1)];
        INSERT INTO chat_posts (course_id, user_id, anonymous_thread_id, title, body, tag, created_at, updated_at)
        VALUES (
          course_id_val,
          sample_user_id,
        ...
        -- Add 1 response
        response_fruit := fruit_names[9 + (course_idx * 2) % array_length(fruit_names, 1)];
        INSERT INTO chat_responses (post_id, user_id, anonymous_thread_id, body, created_at, updated_at)
        VALUES (
          post_record.id,
          sample_user_id,
      ...
      -- Post 5: Problem
      selected_fruit := fruit_names[10 + (course_idx * 2) % array_length(fruit_names, 1)];
        INSERT INTO chat_posts (course_id, user_id, anonymous_thread_id, title, body, tag, created_at, updated_at)
        VALUES (
          course_id_val,
          sample_user_id,
        ...
        -- Add 2 responses
        response_fruit := fruit_names[11 + (course_idx * 2) % array_length(fruit_names, 1)];
        INSERT INTO chat_responses (post_id, user_id, anonymous_thread_id, body, created_at, updated_at)
        VALUES (
          post_record.id,
          sample_user_id,
        ...
        response_fruit := fruit_names[12 + (course_idx * 2) % array_length(fruit_names, 1)];
        INSERT INTO chat_responses (post_id, user_id, anonymous_thread_id, body, created_at, updated_at)
        VALUES (
          post_record.id,
          sample_user_id,
  ...
  RAISE NOTICE 'Sample chat posts seeded successfully!';
END $$;
