...
import { loadCanvasDataForUserId } from "./dataLoader";
...
export async function getCanvasData(): Promise<CanvasData | null> {
  ...
  if (user.email.toLowerCase() === "kare6625@colorado.edu") {
    console.log(
      "[canvasApi] Clearing cache for kare6625@colorado.edu to force fresh load",
    );
  ...
  // Load data from dataset using user ID directly (skip redundant Supabase lookup)
  try {
    console.log(`[canvasApi] User ID: ${user.id}, loading data...`);
    const data = await loadCanvasDataForUserId(
      user.id,
      user.email,
      user.firstName,
    );
...
export async function getCourse(
  courseId: number,
): Promise<CanvasData["courses"][0] | null> {
...
export async function getAssignmentsByCourse(
  courseId: number,
): Promise<CanvasData["assignments"]> {
...
export async function getAssignment(
  assignmentId: number,
): Promise<CanvasData["assignments"][0] | null> {
  const assignments = await getAssignments();
...
export async function getAnnouncementsByCourse(
  courseId: number,
): Promise<CanvasData["announcements"]> {
