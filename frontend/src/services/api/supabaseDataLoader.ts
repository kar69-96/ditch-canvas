...
/**
 * Load Canvas data from Supabase for a specific user
 * Uses the unified extraction_data table with UUID user_id
 */
export async function loadCanvasDataFromSupabase(
  ...
  try {
    ...
    if (courses.length === 0) {
      console.warn(
        `[supabaseDataLoader] No courses found for user ID: ${userId}`,
      );
      return null;
    }

    // Get user info - will be populated from users table elsewhere
    const user = {
    ...
    const coursesData = courses.map((entity) => {
      ...
      return {
        ...
        enrollmentTermId:
          course.enrollmentTermId || course.enrollment_term_id || 1,
        workflowState:
          course.workflowState || course.workflow_state || "available",
      };
    });
    ...
    const assignmentsData = assignments.map((entity) => {
      ...
      const submissionStatus =
        ...
        (assignment.hasSubmission === true
          ? "yes"
          : assignment.hasSubmission === false
            ? "no"
            : null) ||
        null;
      ...
      const assignmentId =
        assignment.id || assignment.assignmentId || entity.entity_id;
      if (assignments.indexOf(entity) < 3) {
        console.log(`[supabaseDataLoader] Sample assignment ${assignmentId}:`, {
          ...
          allKeys: Object.keys(assignment).filter(
            (k) =>
              k.toLowerCase().includes("submission") ||
      ...
      const isQuizFromTypes = (
        assignment.submissionTypes ||
        assignment.submission_types ||
        []
      ).some((type: string) => type.toLowerCase().includes("quiz"));

      return {
        id:
          parseInt(entity.entity_id) ||
          assignment.assignmentId ||
          assignment.id ||
          0,
        title: assignment.title || "Untitled Assignment",
        courseId: entity.course_id
          ? parseInt(entity.course_id)
        ...
        submissionTypes: (assignment.submissionTypes ||
          ...
          []) as string[],
        workflowState:
          assignment.workflowState || assignment.workflow_state || "pending",
    ...
    const quizzesAsAssignments = quizzes.map((entity) => {
      ...
      const dueDate =
        metadata.dueDate || quiz.metadata?.dueDate || quiz.dueDate || "";
      ...
      return {
        ...
        courseId: entity.course_id
          ? parseInt(entity.course_id)
          : parseInt(quiz.courseId) || 0,
        courseName: quiz.courseName || quiz.course_name || "",
        courseCode: quiz.courseCode || quiz.course_code || "",
        dueAt: dueDate,
        assignedAt:
          quiz.assignedAt || quiz.assigned_at || quiz.extractedAt || "",
        pointsPossible:
          metadata.points ||
          quiz.metadata?.points ||
          quiz.pointsPossible ||
          null,
    ...
    const announcementsData = announcements.map((entity) => {
      ...
      if (!postedAt && announcement.postDate) {
        try {
          ...
          const dateMatch = announcement.postDate.match(
    ...
    // Derive bucket name from user ID (UUID format)
    const deriveBucketName = (id: string) => {
      return "user-" + id.replace(/-/g, "").slice(0, 32);
    };
    const defaultBucketName = deriveBucketName(userId);
    ...
    fileBinaries.forEach((entity) => {
      ...
      const originalPath =
        entity.metadata?.originalPath || file.originalPath || "";
      const fileName =
        file.filename ||
        file.fileName ||
        file.name ||
      ...
      const storagePath =
        entity.file_storage_path || file.storagePath || file.storage_path || "";
    ...
    files.forEach((entity) => {
      const file = entity.data as any;
      const courseId = entity.course_id
        ? parseInt(entity.course_id)
        : file.courseId || 0;
      ...
      const key = createFileKey(courseId, fileName);
      const folder =
        entity.metadata?.folderPathString ||
        file.folderPathString ||
        file.folder ||
        "";
      const fileUrl =
        file.url ||
      ...
      if (!fileMap.has(key)) {
        ...
        if (!existing.downloadUrl && file.downloadUrl)
        ...
        if (!existing.canvasFileId && canvasFileId)
          existing.canvasFileId = canvasFileId;
        if (
          (!existing.storageBucket || !existing.storagePath) &&
          file.storagePath
        ) {
    ...
    const findFileForModuleItem = (
      ...
      for (const nameCandidate of nameCandidates) {
        ...
        for (const [fileKey, file] of fileLookupByName.entries()) {
          ...
          if (
            fileKeyParts.length === 2 &&
            fileKeyParts[1].toLowerCase() === candidateLower
          ) {
            return file;
          }
    ...
    const modulesData = modules.map((entity) => {
      ...
      if (module.items && Array.isArray(module.items)) {
        items = module.items.map((item: any) => {
          ...
          const storagePath =
            matchedFile?.storagePath ||
            item.storagePath ||
            item.storage_path ||
            "";
          // Use matched file's bucket, or item's bucket, or default bucket if we have a storage path
          const storageBucket =
            matchedFile?.storageBucket ||
            item.storageBucket ||
            item.storage_bucket ||
            (storagePath ? defaultBucketName : null);
          const originalUrl =
            matchedFile?.originalUrl ||
            downloadUrl ||
            item.originalUrl ||
            item.original_url ||
            "";
          const mimeType =
            matchedFile?.mimeType || item.mimeType || item.mime_type || "";
          const fileName =
            ...
            itemTitle;
          const fileId =
            matchedFile?.canvasFileId || item.fileId || item.file_id || null;
          return {
            id: numericId,
    ...
    if (gradesEntities.length > 0) {
      ...
      gradesData = {
        ...
        semesterProgress:
          grades.semesterProgress || grades.semester_progress || 0,
        courseGrades: (grades.courseGrades ||
    ...
    const assignmentsWithSubmissionStatus = allAssignmentsData.filter(
      (a) => a.submissionStatus !== null && a.submissionStatus !== undefined,
    );
    const submittedAssignments = allAssignmentsData.filter(
      (a) => a.submissionStatus === "yes",
    );
    const quizCount = allAssignmentsData.filter((a) => a.isQuiz).length;
    console.log(
      `[supabaseDataLoader] Successfully loaded Canvas data for user ID: ${userId}`,
    );
    console.log(
      `[supabaseDataLoader] Summary: ${allAssignmentsData.length} total items (${assignmentsData.length} assignments + ${quizCount} quizzes), ${assignmentsWithSubmissionStatus.length} with submission status, ${submittedAssignments.length} marked as submitted`,
    );

    return canvasData;
  } catch (error) {
    console.error(
      `[supabaseDataLoader] Error loading data for user ID ${userId}:`,
      error,
    );
    return null;
  }
