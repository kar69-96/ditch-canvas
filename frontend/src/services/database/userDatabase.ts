...
function supabaseRowToUser(row: any): User {
  return {
    ...
    // Support both old (profile_preferences) and new (preferences) field names
    profilePreferences: row.preferences || row.profile_preferences || {},
    forumData: row.forum_data || {},
...
function userToSupabaseRow(user: Partial<User>): any {
  ...
  if (user.canvasCookiesUpdatedAt !== undefined)
    row.canvas_cookies_updated_at = user.canvasCookiesUpdatedAt;
  if (user.lastLoginAt !== undefined) row.last_login_at = user.lastLoginAt;
  if (user.inviteCodeUsed !== undefined)
    row.invite_code_used = user.inviteCodeUsed;
  if (user.onboardingCompletedAt !== undefined)
    row.onboarding_completed_at = user.onboardingCompletedAt;
  if (user.phoneNumber !== undefined) row.phone_number = user.phoneNumber;
  if (user.profilePreferences !== undefined)
    row.profile_preferences = user.profilePreferences;
  return row;
}
...
export const userDatabase = {
  ...
  async getUserByEmail(email: string): Promise<User | null> {
    try {
      ...
      const res = await fetch(
        `${API_BASE}/api/users/by-email/${encodeURIComponent(normalizedEmail)}`,
      );
      const data = await res.json();
      ...
      if (!res.ok || !data.success) {
        if (res.status === 404) return null;
        console.error(
          "[userDatabase] Error getting user by email:",
          data.error,
        );
        return null;
      }
  ...
  async updateLastLogin(userId: string): Promise<void> {
    try {
      const res = await fetch(
        `${API_BASE}/api/users/${encodeURIComponent(userId)}/last-login`,
        {
          method: "PATCH",
          headers: { "Content-Type": "application/json" },
        },
