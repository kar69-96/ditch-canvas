...
const Dashboard = () => {
  ...
  // Monitor background updates and auto-refresh when complete
  // This continues monitoring if user navigates from Login mid-update
  useBackgroundUpdate({ enabled: true, showToast: false });
    ...
    async function loadUser() {
      ...
      if (session) {
        ...
        if (currentUser) {
          ...
          const displayName =
            currentUser.firstName || currentUser.student || "Student";
  ...
  const calculateSemesterProgress = () => {
    ...
    if (today >= firstSemesterStart && today <= firstSemesterEnd) {
      const totalDays = Math.ceil(
        (firstSemesterEnd.getTime() - firstSemesterStart.getTime()) /
          (1000 * 60 * 60 * 24),
      );
      const daysElapsed = Math.ceil(
        (today.getTime() - firstSemesterStart.getTime()) /
          (1000 * 60 * 60 * 24),
      );
    ...
    if (today >= secondSemesterStart && today <= secondSemesterEnd) {
      const totalDays = Math.ceil(
        (secondSemesterEnd.getTime() - secondSemesterStart.getTime()) /
          (1000 * 60 * 60 * 24),
      );
      const daysElapsed = Math.ceil(
        (today.getTime() - secondSemesterStart.getTime()) /
          (1000 * 60 * 60 * 24),
      );
      return Math.min(100, Math.round((daysElapsed / totalDays) * 100));
    }
  ...
  const dueToday = canvasData.assignments
    ...
    .map((assignment) => {
      const course = canvasData.courses.find(
      ...
      const isCompleted = isAssignmentComplete(
        assignment.id,
                    ...
                    <WeeklyCalendar
                      ...
                      onDayClick={(day) => {
                        if (
                          selectedDay &&
                          day &&
                          selectedDay.fullDate.getTime() ===
                            day.fullDate.getTime()
                        ) {
                          setSelectedDay(null);
                        } else {
                          setSelectedDay(day);
                          ...
                          <div className="p-5 space-y-0">
                            {selectedDay.assignments.map((assignment) => {
                              ...
                              const fullAssignment =
                                canvasData.assignments.find(
                                  (a) => a.id === assignment.id,
                                );
                              const isQuiz =
                                fullAssignment?.isQuiz ||
                                fullAssignment?.submissionTypes?.some((type) =>
                                  type.includes("quiz"),
                                ) ||
                                false;
                              ...
                              const isCompleted =
                                ...
                                isAssignmentComplete(
                                  assignment.id,
                                  fullAssignment?.submissionStatus,
                                );
                              const dueDate = new Date(assignment.dueAt);
                              const formattedDue = (() => {
                                const month = dueDate.toLocaleDateString(
                                  "en-US",
                                  { month: "short" },
                                );
                                const day = dueDate.getDate();
