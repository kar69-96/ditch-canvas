...
import { Calendar as CalendarIcon, List, Clock, FileText, Rocket } from "lucide-react";
...
const Assignments = () => {
  ...
  const quizCount = mockCanvasData.assignments.filter(a => a.isQuiz || a.submissionTypes?.some(type => type.includes("quiz"))).length;
  console.log(`[Assignments] Total items: ${mockCanvasData.assignments.length}, Quizzes: ${quizCount}`);
  if (quizCount > 0) {
    const quizTitles = mockCanvasData.assignments
      .filter(a => a.isQuiz || a.submissionTypes?.some(type => type.includes("quiz")))
      .slice(0, 5)
      .map(a => a.title);
  ...
  const allAssignments = mockCanvasData.assignments.map((assignment) => {
    ...
    try {
      ...
      if (!assignment.dueAt || assignment.dueAt.trim() === '' || isNaN(dueDate.getTime())) {
        hasValidDate = false;
  ...
  const { assignmentsByDay, unknownDateAssignments } = useMemo(() => {
    ...
    const pastDays = dayObjects.filter(d => d.dateKey < today.getTime()).sort((a, b) => b.dateKey - a.dateKey); // Most recent first
    const futureDays = dayObjects.filter(d => d.dateKey > today.getTime()).sort((a, b) => a.dateKey - b.dateKey); // Chronological
  ...
  return (
    <div className="relative w-full">
      ...
      >
        ...
        {/* Header - Sticky */}
        <header 
          className="sticky top-[73px] z-40 bg-background/95 backdrop-blur-sm py-6 sm:py-8 border-b border-border"
        >
